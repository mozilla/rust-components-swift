...
private extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_crashtest_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_crashtest_rustbuffer_free(self, $0) }
    }
}
...
public struct FfiConverterTypeCrashTestError: FfiConverterRustBuffer {
    typealias SwiftType = CrashTestError

    public static func read(from buf: inout (data: Data, offset: Data.Index)) throws -> CrashTestError {
        let variant: Int32 = try readInt(&buf)
        switch variant {
        case 1: return try .ErrorFromTheRustCode(
    ...
    public static func write(_ value: CrashTestError, into buf: inout [UInt8]) {
        switch value {
        case .ErrorFromTheRustCode(_ /* message is ignored*/ ):
...
private var initializationResult: InitializationResult = {
    ...
    if uniffi_crashtest_checksum_func_trigger_rust_abort() != 30701 {
        return InitializationResult.apiChecksumMismatch
    }
    if uniffi_crashtest_checksum_func_trigger_rust_error() != 3305 {
        return InitializationResult.apiChecksumMismatch
    }
    if uniffi_crashtest_checksum_func_trigger_rust_panic() != 21327 {
        return InitializationResult.apiChecksumMismatch
    }
