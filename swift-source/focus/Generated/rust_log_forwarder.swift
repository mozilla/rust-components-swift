...
private enum UniffiCallbackInterfaceAppServicesLogger {
    // Create the VTable using a series of closures.
    // Swift automatically converts these into C callback functions.
    static var vtable: UniffiVTableCallbackInterfaceAppServicesLogger = .init(
        log: { (
            uniffiHandle: UInt64,
            record: RustBuffer,
            ...
            let makeCall = {
                () throws in
                guard let uniffiObj = try? FfiConverterCallbackInterfaceAppServicesLogger.handleMap.get(handle: uniffiHandle) else {
                    throw UniffiInternalError.unexpectedStaleHandle
                }
                return try uniffiObj.log(
                    record: FfiConverterTypeRecord.lift(record)
                )
            }
        ...
        uniffiFree: { (uniffiHandle: UInt64) in
            let result = try? FfiConverterCallbackInterfaceAppServicesLogger.handleMap.remove(handle: uniffiHandle)
            if result == nil {
...
private var initializationResult: InitializationResult = {
    ...
    uniffiCallbackInitAppServicesLogger()
